...
export interface ExchangeConnection {
  ...
  /** Sprint 3.6A.2 — which real ExchangeAccount this credential accesses. Never null in practice once
   * `ensureAccountsBootstrapped()` has run (every connection gets a 1:1 placeholder account the first
   * time it's ever listed) — optional only because a connection can theoretically be read before that
   * bootstrap runs. This is the ownership pointer; `id` above remains just this credential record's
   * own row id, never an account identity. */
  exchangeAccountId?: string | null;
}

/** Sprint 3.6A.2 — Exchange Account Identity Foundation. Formalizes the Core Truth Rule: trading data
 * (Order/Position/Allocation) belongs to a real ExchangeAccount; an ExchangeConnection (API credential)
 * is only ever an access channel to one. Binance's own API does not expose a stable account id this
 * system can rely on in its actual operating environment (verified live 2026-08-10 — Futures Account
 * Info V2/V3 and apiRestrictions carry no uid/accountId; the only known stable id, Spot's `uid`, lives
 * on a different host and is unreachable on Futures TESTNET, since Spot Testnet and Futures Testnet are
 * separate registration/key systems) — so this is a LOCAL identity, established only by explicit user
 * binding (Settings), never by automatic inference from balance/position/symbol/leverage/label
 * similarity. `exchange`+`environment` are frozen at creation and must always match every connection
 * bound to this account (TESTNET and LIVE can never share one ExchangeAccount — see server.ts's
 * binding validation). `preferredConnectionId` is the ONE connection currently used to reach this
 * account's real state; if it becomes unavailable (deleted/disabled), the account is simply
 * unavailable — this app never silently substitutes another bound connection for a write, and reads
 * are treated the same way for this first version (see EXECUTION_SPEC.md's "Exchange Account Identity"
