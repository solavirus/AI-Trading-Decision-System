...
/** Sprint 3.6B.2, Section 8/25 — the account snapshot above deliberately SKIPS querying protective
 * orders for a symbol with zero current position quantity (Task 6's "if no positions: do not call
 * openAlgoOrders at all" — a real, correct optimization for the account-page use case it was built
 * for). But that same scoping makes it structurally unable to ever detect an ORPHANED protection order
 * — one still live on the exchange for a symbol whose position has JUST gone to zero, which is exactly
 * the case External Position Closure Reconciliation needs to check. This is a second, narrowly-scoped
 * caller of the SAME existing `GET /fapi/v1/openAlgoOrders` capability (no new Binance capability),
 * explicit about the one symbol it wants regardless of current position quantity. */
export async function getProtectiveOrdersForSymbol(creds: BinanceFuturesCredentials, symbol: string): Promise<{ ok: true; orders: ExchangeProtectiveOrder[] } | { ok: false; error: ExchangeErrorInfo }> {
  ...
  const { raw, status } = await fetchProtectiveOrdersForSymbols(baseUrl, creds.environment, creds, offset, [toBinanceFuturesSymbol(symbol)]);
  if (status !== 'OK') return { ok: false, error: { code: 'protection_read_unavailable', message: '无法读取保护单状态' } };
  return { ok: true, orders: normalizeProtectiveOrders(raw) };
}
