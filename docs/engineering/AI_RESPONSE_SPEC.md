...
**Addendum (non-version-bumping, 2026-08-12, Sprint 3.7)**: "The AI never creates a Trade" / "Execution creates the Trade only after user confirmation" above describe THIS document's Research → Strategy → Execution workflow specifically — they are unchanged and still govern that workflow exactly as written. Sprint 3.7 introduces a separate, later-authorized `AUTONOMOUS` execution pipeline (see `EXECUTION_SPEC.md`'s "Strategy Order"/"Headless AUTO Execution"/"Autonomous Trading V1" sections) with its own new, narrow, machine-readable decision contract — deliberately NOT a reuse or extension of this `AIResponseRecord`/`ScenarioPlan` contract, and NOT parsed by this document's Parser. That pipeline's own authorization/safety boundary (account-level opt-in, TESTNET-only, the full existing Execution Guard/AUTO Risk Policy/Risk Guard/Kill Switch chain) lives entirely in `EXECUTION_SPEC.md`, not here.
